...
export const SalaryForm = ({
  ...
  const generateExcelReport = () => {
    ...
    const worksheetData = [["Salary Payment Report", "", "", ""], ["Employee Name:", employeeName, "", ""], ["Generated on:", format(new Date(), "MM/dd/yyyy hh:mm a"), "", ""], ["", "", "", ""], ["Date", "Time", "Amount (₹)", "Notes"]];

    // Add records to worksheet
    records.forEach(record => {
      worksheetData.push([record.date, record.time, record.salaryPaid.toString(),
  ...
  return <Card className="w-full animate-fade-in">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="employee-name" placeholder="Enter employee name (e.g., John Doe)" value={employeeName} onChange={e => {
          ...
          }} className={errors.employeeName ? "border-red-500" : ""} aria-label="Employee Name Input" />
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="salary-paid" type="text" placeholder="Enter amount (e.g., 500)" value={salaryPaid} onChange={e => {
            ...
            }} className={`pl-7 ${errors.salaryPaid ? "border-red-500" : ""}`} aria-label="Salary Amount Input" />
            </div>
            {errors.salaryPaid && <p className="text-xs text-red-500 mt-1">{errors.salaryPaid}</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col">
              <input type="file" id="screenshot" ref={fileInputRef} accept="image/png,image/jpeg,image/jpg" onChange={handleImageUpload} className="hidden" aria-label="Upload Transaction Screenshot" />
              <Button type="button" variant="secondary" className="bg-[#4A90E2] text-white hover:bg-[#4A90E2]/90" onClick={() => fileInputRef.current?.click()}>
                Choose File
              </Button>
            </div>
            
            {screenshot && <div className="relative w-24 h-24 mt-2">
                <img src={screenshot} alt="Transaction Screenshot" className="w-24 h-24 object-cover rounded-md" />
                <button type="button" onClick={handleRemoveImage} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 w-6 h-6 flex items-center justify-center">
                  <X className="w-4 h-4" />
                </button>
              </div>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div className="space-y-2">
              ...
              <Input value={format(new Date(), "MM/dd/yyyy")} readOnly disabled className="bg-gray-100" />
            </div>
            <div className="space-y-2">
              <Label className="text-sm font-medium">Time</Label>
              <Input value={format(new Date(), "hh:mm a")} readOnly disabled className="bg-gray-100" />
            </div>
          </div>

          <div className="flex justify-center pt-2">
            <Button type="submit" className="w-full transition-transform hover:scale-105 duration-300">
      ...
      <CardFooter className="flex flex-col space-y-2">
        <div className="flex justify-between w-full">
          ...
          <Button type="button" variant="outline" onClick={shareViaWhatsApp} className="flex-1 ml-2">
            <Share className="mr-2 h-4 w-4" />
            Share
          </Button>
        </div>
        
        <Button type="button" variant="secondary" onClick={onViewRecords} className="w-full border border-[#4A90E2] text-[#4A90E2] bg-white hover:bg-gray-50">
